// @TO-DO Default value of SCHEMA NEED TO BE UPDATED
...
export default class Realm {
  constructor(params) {
    ...
    params.schema.forEach((schema) => {
      this.data[schema.schema.name] = {};
    });
    params.schema.forEach((schema) => {
      this.schema[schema.schema.name] = schema.schema;
    });
  ...
  create(schemaName, object) {
    ...
    if (modelObject._id) {
      this.data[schemaName][modelObject._id] = modelObject;
    } else {
      this.data[schemaName][Object.keys(modelObject)[0]] = modelObject;
    }
